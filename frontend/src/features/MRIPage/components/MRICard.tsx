...
import {analyzeMRI, AnalyzeMRIResponse} from "../../../api/ml/analyzeMri";

type XRayCardProps = {
    id: number
    url: string,
    date: string,
    openModal: (analysis: AnalyzeMRIResponse) => void
}

export const MRICard = ({id, url, date, openModal}: XRayCardProps) => {

    const navigate = useNavigate();

    const onAnalyze = async () => {
        try {
            const response = await analyzeMRI({
